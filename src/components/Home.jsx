...
const Home = () => {
  ...
  async function getDefaultPhotos() {
    const data = await fetch(`${API_URL_1}?client_id=${import.meta.env.VITE_UNSPLASH_KEY}`)
  ...
  console.log(defaultPhotos);
   ...
   async function getPhotos() {
     try {
       const { data } = await axios.get(`${API_URL}?query=${searchInput.current.value}&page=1&per_page=20&client_id=${import.meta.env.VITE_UNSPLASH_KEY}`)
  ...
  return (
    <div className='bg-bg-orange w-full h-max p-8 | flex flex-col justify-center items-center gap-8'>
      ...
      <h1 className='text-black text-2xl'>Drag and drop any picture at your own convenience</h1>
      <Search 
